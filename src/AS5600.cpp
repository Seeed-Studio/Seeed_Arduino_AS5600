...
/***************************************************/
AMS_5600::AMS_5600(TwoWire &which_wire)
{
  /* set i2c address */ 
...
{
  int retVal = -1;
  _pwire->beginTransmission(_ams5600_Address);
  _pwire->write(in_adr);
  _pwire->endTransmission();
  _pwire->requestFrom(_ams5600_Address, 1);
...
word AMS_5600::readTwoBytes(int in_adr_hi, int in_adr_lo)
{
  ...
  /* Read High Byte */  
  _pwire->beginTransmission(_ams5600_Address);
  _pwire->write(in_adr_hi);
  _pwire->endTransmission();
  _pwire->requestFrom(_ams5600_Address, 1);
  
  while(_pwire->available() == 0);
  
  word high = _pwire->read();
